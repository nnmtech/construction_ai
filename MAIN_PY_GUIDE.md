# app/main.py - Complete FastAPI Application Guide

## Overview

The `app/main.py` file is the central entry point of the Construction AI Landing Page application. It:

1. **Initializes FastAPI** with configuration and metadata
2. **Sets up middleware** for CORS, compression, and security
3. **Initializes the database** with SQLAlchemy models
4. **Includes all route blueprints** (forms, ROI, booking)
5. **Serves static files** (CSS, JavaScript, images)
6. **Implements error handling** for exceptions and validation errors
7. **Provides health checks** for monitoring and orchestration
8. **Manages application lifecycle** with startup/shutdown events
...
**Purpose:** Configures logging to output to both console and file for debugging and monitoring.
...
**Purpose:** Creates all database tables before the application starts. Fails fast if database is not configured.
...
**Purpose:** Creates the FastAPI instance with metadata and documentation endpoints.
...
**Purpose:** Registers all route blueprints, making endpoints available.
...
**Purpose:** Provides monitoring endpoints for health checks and container orchestration.
...
**Purpose:** Manages application lifecycle and resource cleanup.

---

## Configuration Loading

The application loads configuration from `.env` file through `app/config.py`:
...
settings.smtp_server           # Email server
settings.cost_per_day_delay    # ROI calculator constant
...
| `/api/info` | GET | Comprehensive app info |
...
2. **Static file caching** - Browser caching for assets
3. **Connection pooling** - Database connection reuse
4. **Async/await** - Non-blocking I/O
5. **Background tasks** - Email sending in background
...
- **Application info** - `/api/info` endpoint
- **API documentation** - `/api/docs` and `/api/redoc`
- **Endpoint listing** - `/api/endpoints`

---

## Summary

The `app/main.py` file is the core of the FastAPI application. It:
...
This is a production-ready FastAPI application that can be deployed to any environment.
